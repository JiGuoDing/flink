...
2. ? 拼 Key 后从左流状态中查找所有对应订单记录。
3. 过滤出用户 id 为 U123 的订单记录。

预期查询过程：

1. 右流到达用户 id 为 U123 的用户信息更新记录。
2. 从 Join Key 层查找 U123 对应的订单 id 指针及偏移量 (PS-Tree)。
3. 验证订单 id 有效性后从 Primary Key 层顺序读取所有订单 id。
4. 从 RocksDB 中读取对应订单记录。
